let randomNumber1 = Math.floor(Math.random() * 6) + 1;
let randImage1 = "images/dice" + randomNumber1 + ".png";
document.querySelector(".img")[0].setAttribute("src", randImage1);
let randomNumber2 = Math.floor(Math.random() * 6) + 1;
let randImage2 = "images/dice" + randomNumber2 + ".png";
document.querySelector(".img")[1].setAttribute("src", randImage2);

if (randomNumber1 > randomNumber2) {
  document.querySelector("h1").innerHTML = " Player 1 Pobijedio!";
} else if (randomNumber2 > randomNumber1) {
  document.querySelector("h1").innerHTML = " Player 2 Pobijedio!";
} else {
  document.querySelector("h1").innerHTML = "Neriješeno!";
}
